...
RW_TOKENS=$(PRIVKEY=$CG_PRIVKEY COSMIC_GAME_ADDRESS=$CONTRACT_ADDR npx hardhat run scripts/mint_rwalks.js --network localhost)
echo Random walk tokens: $RW_TOKENS
RWALK_TOKENS=$RW_TOKENS PRIVKEY=$CG_PRIVKEY COSMIC_GAME_ADDRESS=$CONTRACT_ADDR npx hardhat run scripts/test-deployment-part1.js --network localhost
PRIVKEY=$CG_PRIVKEY COSMIC_GAME_ADDRESS=$CONTRACT_ADDR npx hardhat run scripts/forward-time-past-claim-prize.js --network localhost
