...
use three_body_problem::{
    render::{
        ...
        effects::{EffectConfig, EffectChainBuilder, FrameParams, DogBloomConfig},
        types::{BlurConfig, Resolution, ChannelLevels},
    },
...
fn test_effect_chain_disabled_effects() {
    ...
    let output = result.unwrap();
    let total_diff: f64 = input.iter().zip(output.iter())
        .map(|(inp, out)| (inp.0 - out.0).abs())
        .sum();
