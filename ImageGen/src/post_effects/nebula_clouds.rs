...
mod tests {
    ...
    fn test_disabled_when_zero_strength() {
        let config =
            NebulaCloudConfig { strength: 0.0, ..NebulaCloudConfig::special_mode(1920, 1080, 0) };
        let nebula = NebulaClouds::new(config);
    ...
    #[test]
    fn test_special_mode_defaults_are_subtle_and_smooth() {
        let config = NebulaCloudConfig::special_mode(1920, 1080, 42);
        assert!(
            (0.02..=0.12).contains(&config.strength),
            "Special-mode nebula strength should stay subtle: {}",
            config.strength
        );
    ...
    #[test]
    fn test_adjacent_frames_change_gradually() {
        let config = NebulaCloudConfig::special_mode(96, 54, 42);
        let nebula = NebulaClouds::new(config);

        let input = vec![(0.0, 0.0, 0.0, 0.0); 96 * 54];
        let frame_0 = nebula.process_with_time(&input, 96, 54, 120).unwrap();
        let frame_1 = nebula.process_with_time(&input, 96, 54, 121).unwrap();

        let mean_delta = frame_0
            .iter()
            .zip(frame_1.iter())
            .map(|(a, b)| (a.0 - b.0).abs() + (a.1 - b.1).abs() + (a.2 - b.2).abs())
            .sum::<f64>()
            / (frame_0.len() as f64 * 3.0);

        assert!(mean_delta > 0.0, "Nebula should evolve over time");
        assert!(
            mean_delta < 0.01,
            "Nebula should evolve gradually between adjacent frames: {mean_delta}"
        );
    }
}
