...
mod tests {
    ...
    fn test_wide_range_randomization() {
        let config = RandomizableEffectConfig { gallery_quality: false, ..Default::default() };
        ...
        assert!(
            resolved.chromatic_bloom_strength >= 0.10 && resolved.chromatic_bloom_strength <= 1.0
        );
        ...
        assert!(
            resolved.nebula_strength >= 0.02 && resolved.nebula_strength <= 0.12,
            "Nebula strength {} out of subtle range",
            resolved.nebula_strength
        );
        assert!(resolved.nebula_octaves >= 2 && resolved.nebula_octaves <= 7);
        assert!(
            resolved.nebula_base_frequency >= 0.0005 && resolved.nebula_base_frequency <= 0.0035,
            "Nebula base frequency {} out of range",
            resolved.nebula_base_frequency
        );
    ...
    #[test]
    fn test_nebula_randomization_is_subtle_across_many_seeds() {
        let config = RandomizableEffectConfig { gallery_quality: false, ..Default::default() };

        for seed_val in 1..80 {
            let seed = [seed_val, 9, 8, 7, 6, 5, 4, 3];
            let mut rng = Sha3RandomByteStream::new(&seed, 100.0, 300.0, 300.0, 1.0);
            let (resolved, _log) = config.resolve(&mut rng, 1920, 1080, true);
            assert!(
                (0.02..=0.12).contains(&resolved.nebula_strength),
                "Nebula strength out of subtle bounds: {}",
                resolved.nebula_strength
            );
        }
    }
