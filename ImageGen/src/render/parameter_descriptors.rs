...
impl FloatParamDescriptor {
    ...
    pub fn range(&self, gallery_quality: bool) -> (f64, f64) {
        if gallery_quality { (self.gallery_min, self.gallery_max) } else { (self.min, self.max) }
    }
}
...
pub const NEBULA_STRENGTH: FloatParamDescriptor = FloatParamDescriptor {
    ...
    min: 0.02, // Subtle cosmic atmosphere
    max: 0.12, // Intentionally capped for elegant, non-overpowering backgrounds
    gallery_min: 0.04,
    gallery_max: 0.09,
