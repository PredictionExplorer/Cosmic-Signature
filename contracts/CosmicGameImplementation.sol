	...
	CosmicGameStorage
{
	...
	// New events for admin functions
	...
	function _distributeRafflePrizes(uint256 raffleAmount_) internal {
		...
		for (uint256 i = 0; i < numRaffleETHWinnersBidding; i++) {
			...
			require(
				success,
				CosmicGameErrors.FundTransferFailed("Transfer to raffle winner failed.", perWinnerAmount, raffleWinner)
			);
	...
	// NFT Donation and Claiming Functions

	function donateNFT(IERC721 nftAddress, uint256 tokenId) external nonReentrant whenNotPaused {
		require(
			systemMode < CosmicGameConstants.MODE_MAINTENANCE,
			CosmicGameErrors.SystemMode(CosmicGameConstants.ERR_STR_MODE_RUNTIME, systemMode)
		);
	...
	function claimDonatedNFT(uint256 index) external nonReentrant whenNotPaused {
		require(
			systemMode < CosmicGameConstants.MODE_MAINTENANCE,
			CosmicGameErrors.SystemMode(CosmicGameConstants.ERR_STR_MODE_RUNTIME, systemMode)
		);
	...
	function claimManyDonatedNFTs(uint256[] calldata indices) external nonReentrant whenNotPaused {
	...
	function setBusinessLogicContract(address _bLogic) external onlyOwner {
		require(_bLogic != address(0), "Invalid address");
		bLogic = _bLogic;
		emit BusinessLogicAddressChanged(_bLogic);
	}
