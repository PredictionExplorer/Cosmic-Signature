...
/// @notice V2 bidding interface with time-based CST bid rewards and front-running slippage protection.
/// @dev V2 replaces the V1 bid function signatures (it does not extend `IBidding`); each bid function
/// gains a `cstBidRewardMinLimit_` parameter so the bidder can require a minimum CST reward.
...
interface IBiddingV2 is ICosmicSignatureGameStorage, IBiddingBase, IMainPrizeBase, IBidStatistics {
	...
	/// slippage protection because they have no parameter slot for `cstBidRewardMinLimit_`.
	...
	/// [/Comment-202503151]
	/// @param cstBidRewardMinLimit_ Comment-V2-CST-MIN-LIMIT applies.
	function bidWithEthAndDonateToken(int256 randomWalkNftId_, string memory message_, uint256 cstBidRewardMinLimit_, IERC20 tokenAddress_, uint256 amount_) external payable;
	...
	/// [/Comment-202503153]
	/// @param cstBidRewardMinLimit_ Comment-V2-CST-MIN-LIMIT applies.
	function bidWithEthAndDonateNft(int256 randomWalkNftId_, string memory message_, uint256 cstBidRewardMinLimit_, IERC721 nftAddress_, uint256 nftId_) external payable;
	...
	/// [/Comment-202503155]
	/// @param cstBidRewardMinLimit_ .
	/// [Comment-V2-CST-MIN-LIMIT]
	/// Minimum CST reward in wei the bidder is willing to accept for placing this bid.
	/// If the actual reward (computed at execution time as `floor(sqrt(3 * elapsedSeconds) * 1e18)`)
	/// is less than this value, the call reverts with `CstBidRewardMinLimitNotReached`.
	...
	/// [/Comment-V2-CST-MIN-LIMIT]
	function bidWithEth(int256 randomWalkNftId_, string memory message_, uint256 cstBidRewardMinLimit_) external payable;
	...
	/// @param cstBidRewardMinLimit_ Comment-V2-CST-MIN-LIMIT applies.
	function bidWithCstAndDonateToken(uint256 priceMaxLimit_, string memory message_, uint256 cstBidRewardMinLimit_, IERC20 tokenAddress_, uint256 amount_) external;

	/// @notice Places a CST bid and donates an NFT in a single transaction.
	/// Comment-202503168 applies.
	/// Comment-202503153 applies.
	/// @param cstBidRewardMinLimit_ Comment-V2-CST-MIN-LIMIT applies.
	function bidWithCstAndDonateNft(uint256 priceMaxLimit_, string memory message_, uint256 cstBidRewardMinLimit_, IERC721 nftAddress_, uint256 nftId_) external;
	...
	/// @param cstBidRewardMinLimit_ Comment-V2-CST-MIN-LIMIT applies.
	function bidWithCst(uint256 priceMaxLimit_, string memory message_, uint256 cstBidRewardMinLimit_) external;
