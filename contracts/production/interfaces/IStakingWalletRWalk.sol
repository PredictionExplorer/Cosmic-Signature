...
interface IStakingWalletRWalk {
	...
	/// @param nftId The ID of the staked token
	...
	/// @param stakeActionId The ID of the unstake action
	/// @param nftId The ID of the unstaked token
	/// @param totalNFTs Total number of staked NFTs after this action
	/// @param staker Address of the staker
	...
	/// @param nftId_ ID of the token to stake
	...
	/// @param nftId_ ID of the token to check
	...
	function isTokenStaked(uint256 nftId) external view returns (bool);
	...
	function lastActionIdByTokenId(uint256 nftId) external view returns (int256);

	/// @notice Gets the staker's address for a given token
	/// @param nftId ID of the token to check
	/// @return Address of the staker, address(0) if not staked
	function stakerByTokenId(uint256 nftId) external view returns (address);
