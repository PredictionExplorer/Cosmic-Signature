...
describe("Cosmic", function () {
	...
	describe("Deployment", function () {
		...
		it("Should be possible to bid", async function () {
			...
			await expect(
				cosmicGame.connect(addr1).bid(["", ethers.BigNumber.from("-1")], { value: 1 }),
			).to.be.revertedWith("Call to bid logic failed.");
			let bidPrice = await cosmicGame.getBidPrice();
			await expect(
				cosmicGame.connect(addr1).bid(["", ethers.BigNumber.from("-1")], { value: bidPrice.sub(1) }),
			).to.be.revertedWith("Call to bid logic failed.");
		...
		it("Should be possible to bid with RandomWalk token", async function () {
			...
			await expect(
				cosmicGame.connect(owner).bid(["", ethers.BigNumber.from("0")], { value: bidPrice }),
			...
			bidPrice = await cosmicGame.getBidPrice();
			await expect(
				cosmicGame.connect(owner).bid(["", ethers.BigNumber.from(token_id)], { value: bidPrice }),
			).to.be.revertedWith("Call to bid logic failed."); //tokenId=0
		});
		...
		it("claimManyDonatedNFTs() works properly", async function () {
			...
			await cosmicGame
				.connect(addr1)
				.bidAndDonateNFT(["", ethers.BigNumber.from("-1")], randomWalkNFT.address, 1, { value: bidPrice });
