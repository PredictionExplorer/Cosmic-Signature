...
describe("Bidding-Old", function () {
	...
	it("Long-term aggressive bidding behaves correctly", async function () {
		...
		// 	console.info(latestBlockTimeStamp);
			...
			if (timeIncrease > 0) {
				...
				for (const transaction of transactions) {
					try {
						...
						// console.info("Success 1.", transactions.length);
					} catch (error2Details) { // todo-1 Rename `error2Details` to `error2Object`?
						// console.error("Error.", transactions.length);

						// console.error();
						// console.error(error2Details.message);
						// console.error(error2Details);
						...
						if (error2Details.message.endsWith(", but it didn't revert")) {
							// console.info("Success 2.", transactions.length);
