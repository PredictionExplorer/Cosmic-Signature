...
describe("Security", function () {
	...
	it("Vulnerability to claimPrize() multiple times", async function () {
		...
		await cosmicGameProxy.setTokenContract(cosmicToken.address);
		await cosmicGameProxy.setNftContract(cosmicSignature.address);
		await cosmicGameProxy.setCharity(charityWallet.address);
		await cosmicGameProxy.setRaffleWallet(raffleWallet.address);
		await cosmicGameProxy.setRandomWalk(randomWalkNFT.address);
		await cosmicGameProxy.setActivationTime(0);
		let prizePercentage = "10";
		await cosmicGameProxy.setPrizePercentage(ethers.BigNumber.from(prizePercentage));
		await cosmicGameProxy.setRuntimeMode();

		const ReClaim = await ethers.getContractFactory("ReClaim");
		const reclaim = await ReClaim.deploy(cosmicGameProxy.address);
