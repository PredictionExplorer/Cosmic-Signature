...
describe("StakingWalletCosmicSignatureNft-Old", function () {
	...
	// 	const newStakingWalletCosmicSignatureNft_ = await stakingWalletCosmicSignatureNftFactory.deploy(bidderContractAddr_, signer0.address, brokenCharityAddr);
	// 	await newStakingWalletCosmicSignatureNft_.waitForDeployment();
	// 	await newStakingWalletCosmicSignatureNft_.transferOwnership(ownerAcct.address);
	//
	// 	await expect(
	// 		// todo-9 This method now has parameters.
	// 		newStakingWalletCosmicSignatureNft_.>>>connect>>>.deposit({ value: hre.ethers.parseEther("2") })
	// 	).revertedWithCustomError(newStakingWalletCosmicSignatureNft_, "FundTransferFailed");
	// });
	...
	it("It's impossible to unstake an NFT twice", async function () {
		...
		await newStakingWalletCosmicSignatureNft_.waitForDeployment();
		const newStakingWalletCosmicSignatureNftAddr = await newStakingWalletCosmicSignatureNft_.getAddress();
		await newStakingWalletCosmicSignatureNft_.transferOwnership(ownerAcct.address);
		await newCosmicSignatureNft.connect(signer0).setApprovalForAll(newStakingWalletCosmicSignatureNftAddr, true);

		const transactionResponse_ = await newStakingWalletCosmicSignatureNft_.connect(signer0).stakeMany([0, 1]);
		const transactionReceipt_ = await transactionResponse_.wait();
		// const topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("NftStaked").topicHash;
		// const receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
		// const log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);

		// expect(await newStakingWalletCosmicSignatureNft_.lastActionIdByTokenId(0)).equal(0);
		// expect(await newStakingWalletCosmicSignatureNft_.stakerByTokenId(0)).equal(signer0.address);
		// expect(await newStakingWalletCosmicSignatureNft_.stakerByTokenId(99n)).equal(hre.ethers.ZeroAddress);
		...
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).unstake(1)).not.to.be.reverted;
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).unstake(1)).revertedWithCustomError(
			newStakingWalletCosmicSignatureNft_,
			"NftStakeActionInvalidId"
		);
		expect(await newStakingWalletCosmicSignatureNft_.usedNfts(0n)).equal(1n);

		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).deposit(0, { value: hre.ethers.parseEther("1") })).not.to.be.reverted;
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).deposit(1, { value: hre.ethers.parseEther("2") })).not.to.be.reverted;
		// expect(await newStakingWalletCosmicSignatureNft_.numEthDeposits()).equal(1n);
		// const d = await newStakingWalletCosmicSignatureNft_.ethDeposits(1);
		// expect(d.rewardAmountPerStakedNft).equal(hre.ethers.parseEther("3"));
	});
	...
	it("Shouldn't be possible to unstake by a user different from the owner", async function () {
		...
		const newStakingWalletCosmicSignatureNft_ = await stakingWalletCosmicSignatureNftFactory.deploy(
		...
		const transactionResponse_ = await newStakingWalletCosmicSignatureNft_.connect(signer0).stake(0);
		const transactionReceipt_ = await transactionResponse_.wait();
		// const topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("NftStaked").topicHash;
		// const receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
		// const log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
		...
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer1).unstake(1)).revertedWithCustomError(
			newStakingWalletCosmicSignatureNft_,
	...
	//
	// 	const transactionResponse_ = await newStakingWalletCosmicSignatureNft_.>>>connect>>>.stake(0);
	// 	const transactionReceipt_ = await transactionResponse_.wait();
	// 	const topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("NftStaked").topicHash;
	// 	const receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
	// 	// const log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
	// 	// const unstakeTime = log.args.unstakeTime;
	// 	const numStakedNfts = await newStakingWalletCosmicSignatureNft_.numStakedNfts();
	// 	expect(numStakedNfts).equal(1);
	// 	await hre.ethers.provider.send("evm_increaseTime", [6000]);
	// 	// await hre.ethers.provider.send("evm_mine");
	...
	//
	// 	await expect(newStakingWalletCosmicSignatureNft_.claimManyRewards([1], [0])).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"NftNotUnstaked"
	// 	);
	// });
	...
	//
	// 	await expect(cosmicSignatureGameProxy.>>>connect>>>.depositToStakingWalletCosmicSignatureNft({ value: hre.ethers.parseEther("2") })).not.to.be.reverted;
	// 	await expect(newStakingWalletCosmicSignatureNft_.>>>connect>>>.unstake(1)).not.to.be.reverted;
	// 	await expect(newStakingWalletCosmicSignatureNft_.claimManyRewards([1],[0, 0])).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"IncorrectArrayArguments"
	// 	);
	// 	await expect(newStakingWalletCosmicSignatureNft_.claimManyRewards([1], [0])).not.to.be.reverted;
	// 	await expect(newStakingWalletCosmicSignatureNft_.claimManyRewards([1], [0])).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	...
	//
	// 	await expect(cosmicSignatureGameProxy.>>>connect>>>.depositToStakingWalletCosmicSignatureNft({ value: hre.ethers.parseEther("2") })).not.to.be.reverted;
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.unstake(1);
	// 	await expect(newStakingWalletCosmicSignatureNft_.connect(signer1).claimManyRewards([1], [0])).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"NftStakeActionAccessDenied"
	// 	);
	// });
	...
	//
	// 	await newStakingWalletCosmicSignatureNft_.connect(signer1).stake(1);
	// 	await expect(cosmicSignatureGameProxy.>>>connect>>>.depositToStakingWalletCosmicSignatureNft({ value: hre.ethers.parseEther("2") })).not.to.be.reverted;
	// 	// await hre.ethers.provider.send("evm_mine");
	// 	const transactionResponse_ = await newStakingWalletCosmicSignatureNft_.>>>connect>>>.stake(0);
	// 	const transactionReceipt_ = await transactionResponse_.wait();
	...
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.unstake(1);
	// 	await expect(newStakingWalletCosmicSignatureNft_.claimManyRewards([3], [0])).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	...
	// 	await newCosmicSignatureNft.connect(signer1).setApprovalForAll(newStakingWalletCosmicSignatureNftAddr, true);
	//
	// 	let transactionResponse_ = await newStakingWalletCosmicSignatureNft_.>>>connect>>>.stake(0);
	// 	let transactionReceipt_ = await transactionResponse_.wait();
	// 	let topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("NftStaked").topicHash;
	// 	let receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
	// 	// let log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
	// 	// const unstakeTime = log.args.unstakeTime;
	// 	const stakeRecord = await newStakingWalletCosmicSignatureNft_.stakeActions(1);
	// 	const numStakeActions = await newStakingWalletCosmicSignatureNft_.numStakeActions();
	// 	const stakeTime = stakeRecord.stakeTime;
	// 	await newStakingWalletCosmicSignatureNft_.connect(signer1).stake(1); // we need to stake, otherwise the deposit would be rejected
	// 	await hre.ethers.provider.send("evm_increaseTime", [6000]);
	// 	// await hre.ethers.provider.send("evm_mine");
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.unstake(1);
	// 	await hre.ethers.provider.send("evm_increaseTime", [6000]);
	...
	// 	topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("EthDepositReceived").topicHash;
	// 	receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
	// 	// log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
	// 	await expect(newStakingWalletCosmicSignatureNft_.claimManyRewards([1], [0])).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"DepositOutsideStakingWindow"
	// 	);
	...
	it("Shouldn't be possible to unstake with invalid stakeActionId", async function () {
		...
		let transactionResponse_ = await newStakingWalletCosmicSignatureNft_.connect(signer0).stake(0);
		let transactionReceipt_ = await transactionResponse_.wait();
		// let topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("NftStaked").topicHash;
		// let receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
		// let log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
		// const unstakeTime = log.args.unstakeTime;
		// const stakeRecord = await newStakingWalletCosmicSignatureNft_.stakeActions(1);
		// let numStakeActions = await newStakingWalletCosmicSignatureNft_.numStakeActions();
		// const stakeTime = stakeRecord.stakeTime;
		await newStakingWalletCosmicSignatureNft_.connect(signer1).stake(1); // we need to stake, otherwise the deposit would be rejected
		await hre.ethers.provider.send("evm_increaseTime", [6000]);
		// await hre.ethers.provider.send("evm_mine");
		...
		// receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
		// log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
		// numStakeActions = await newStakingWalletCosmicSignatureNft_.numStakeActions();
		// const numDeposits = await newStakingWalletCosmicSignatureNft_.numEthDeposits();
		// await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).claimManyRewards([/*numStakeActions*/ 3], [0])).revertedWithCustomError(
		// 	newStakingWalletCosmicSignatureNft_,
		// 	"NftStakeActionInvalidId"
		// );
		// await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).claimManyRewards([1], [numDeposits])).revertedWithCustomError(
		// 	newStakingWalletCosmicSignatureNft_,
		// 	"EthDepositInvalidId"
		// );
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).unstakeMany([0])).revertedWithCustomError(
			newStakingWalletCosmicSignatureNft_,
			"NftStakeActionInvalidId"
		);
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).unstakeMany([3])).revertedWithCustomError(
			newStakingWalletCosmicSignatureNft_,
			"NftStakeActionInvalidId"
		);
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer1).unstakeMany([10])).revertedWithCustomError(
			newStakingWalletCosmicSignatureNft_,
			"NftStakeActionInvalidId"
		);
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer1).unstakeMany([2])).not.to.be.reverted;
	});
	...
	it("It is not possible to unstake if transfer to sender address fails", async function () {
		...
		const brokenStakingWalletCosmicSignatureNft_ = await brokenStakingWalletCosmicSignatureNftFactory.deploy();
		await brokenStakingWalletCosmicSignatureNft_.waitForDeployment();
		const brokenStakingWalletCosmicSignatureNftAddr = await brokenStakingWalletCosmicSignatureNft_.getAddress();
		// await brokenStakingWalletCosmicSignatureNft_.transferOwnership(ownerAcct.address);
		await brokenStakingWalletCosmicSignatureNft_.connect(ownerAcct).setStakingWalletCosmicSignatureNft(newStakingWalletCosmicSignatureNftAddr);
		await brokenStakingWalletCosmicSignatureNft_.connect(signer0).doSetApprovalForAll(newCosmicSignatureNftAddr);
		...
		let transactionResponse_ = await brokenStakingWalletCosmicSignatureNft_.connect(signer0).doStake(0);
		let transactionReceipt_ = await transactionResponse_.wait();
		// let topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("NftStaked").topicHash;
		// let receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
		// let log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
		...
		// const stakeRecord = await newStakingWalletCosmicSignatureNft_.stakeActions(1);
		// const stakeTime = stakeRecord.stakeTime;
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer1).stake(1)).not.to.be.reverted;
		await expect(brokenStakingWalletCosmicSignatureNft_.connect(signer0).doStake(2n)).not.reverted;
		...
		await expect(brokenStakingWalletCosmicSignatureNft_.connect(signer0).doUnstake(3)).not.to.be.reverted;
		await brokenStakingWalletCosmicSignatureNft_.connect(signer0).startBlockingDeposits();
		// await expect(brokenStakingWalletCosmicSignatureNft_.connect(signer0).doClaimReward(1, 0)).revertedWithCustomError(
		// 	brokenStakingWalletCosmicSignatureNft_,
		// 	"FundTransferFailed"
		// );
		await expect(brokenStakingWalletCosmicSignatureNft_.connect(signer0).doUnstake(1)).revertedWithCustomError(
			newStakingWalletCosmicSignatureNft_,
			"FundTransferFailed"
		);
		await brokenStakingWalletCosmicSignatureNft_.connect(signer0).stopBlockingDeposits();
		await expect(brokenStakingWalletCosmicSignatureNft_.connect(signer0).doUnstake(1)).not.to.be.reverted;
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer1).unstake(2)).not.to.be.reverted;
		await expect(brokenStakingWalletCosmicSignatureNft_.connect(signer0).doUnstake(1)).revertedWithCustomError(
			newStakingWalletCosmicSignatureNft_,
			"NftStakeActionInvalidId"
		);
	...
	// 	expect(charityAddr).equal(signer3.address);
	...
	// 	const newStakingWalletCosmicSignatureNft_ = await testStakingWalletCosmicSignatureNftFactory.deploy(cosmicSignatureNftAddr, cosmicSignatureGameProxyAddr);
	// 	await newStakingWalletCosmicSignatureNft_.waitForDeployment();
	// 	await newStakingWalletCosmicSignatureNft_.transferOwnership(ownerAcct.address);
	// 	await cosmicSignatureGameProxy.connect(ownerAcct).setStakingWalletCosmicSignatureNft(newStakingWalletCosmicSignatureNftAddr);
	//
	...
	// 	let tokenStaked = await newStakingWalletCosmicSignatureNft_.isTokenStaked(sampleTokenId);
	// 	expect(tokenStaked).equal(false);
	// 	await newStakingWalletCosmicSignatureNft_.doInsertToken(sampleTokenId, 0n);
	// 	let tokenIndexCheck = await newStakingWalletCosmicSignatureNft_.tokenIndexes(sampleTokenId);
	// 	expect(tokenIndexCheck).equal(1);
	// 	let tokenIdCheck = await newStakingWalletCosmicSignatureNft_.stakedTokens(Number(tokenIndexCheck) - 1);
	// 	expect(tokenIdCheck).equal(sampleTokenId);
	// 	await expect(newStakingWalletCosmicSignatureNft_.doInsertToken(sampleTokenId, 0n)).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"TokenAlreadyInserted"
	// 	);
	//
	// 	await newStakingWalletCosmicSignatureNft_.doRemoveToken(sampleTokenId);
	// 	await expect(newStakingWalletCosmicSignatureNft_.doRemoveToken(sampleTokenId)).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"TokenAlreadyDeleted"
	// 	);
	//
	...
	// 	// const log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
	//
	// 	await expect(newStakingWalletCosmicSignatureNft_.doInsertToken(0, 0)).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"TokenAlreadyInserted"
	// 	);
	// 	let numStakedNfts_ = await newStakingWalletCosmicSignatureNft_.numStakedNfts();
	// 	expect(numStakedNfts_).equal(1);
	// 	await hre.ethers.provider.send("evm_increaseTime", [6000]);
	// 	// await hre.ethers.provider.send("evm_mine");
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.unstake(1);
	// 	numStakedNfts_ = await newStakingWalletCosmicSignatureNft_.numStakedNfts();
	// 	expect(numStakedNfts_).equal(0);
	// 	await expect(newStakingWalletCosmicSignatureNft_.doRemoveToken(0)).revertedWithCustomError(
	// 		newStakingWalletCosmicSignatureNft_,
	// 		"TokenAlreadyDeleted"
	// 	);
	//
	...
	// 	// const m = await newStakingWalletCosmicSignatureNft_.modulo();
	...
	it("User stakes his 10 Cosmic Signature NFTs and gets all of them back after unstake", async function () {
		...
		let numStakedNfts = await newStakingWalletCosmicSignatureNft_.numStakedNfts();
		expect(numStakedNfts).equal(0);

		for (let i = 0; i < 10; i++) {
			const nftId_ = ((i * 7) + 2) % 10;
			const transactionResponse_ = await newStakingWalletCosmicSignatureNft_.connect(signer0).stake(nftId_);
			const transactionReceipt_ = await transactionResponse_.wait();
			// const topic_sig = newStakingWalletCosmicSignatureNft_.interface.getEvent("NftStaked").topicHash;
			// const receipt_logs = transactionReceipt_.logs.filter((log_) => (log_.topics.indexOf(topic_sig) >= 0));
			// const log = newStakingWalletCosmicSignatureNft_.interface.parseLog(receipt_logs[0]);
		...
		numStakedNfts = await newStakingWalletCosmicSignatureNft_.numStakedNfts();
		expect(numStakedNfts).equal(10);

		for ( let nftId_ = 0; nftId_ < 10; ++ nftId_ ) {
			const o = await newCosmicSignatureNft.ownerOf(nftId_);
			expect(o).equal(newStakingWalletCosmicSignatureNftAddr);
		}
		...
		for ( let stakeActionId_ = 5; stakeActionId_ <= 10; ++ stakeActionId_ ) {
			await newStakingWalletCosmicSignatureNft_.connect(signer0).unstake(stakeActionId_);
		}
		await newStakingWalletCosmicSignatureNft_.connect(signer0).unstakeMany([3, 4, 2]);
		await newStakingWalletCosmicSignatureNft_.connect(signer0).unstake(1);
	...
	// 	expect(parsed_log.args.amount).equal(prevCosmicSignatureNftStakingTotalEthRewardAmount_);
	// 	expect(parsed_log.args.modulo).equal(moduloInRound);
	//
	// 	// todo-0 This contract no longer exists.
	// 	const brokenCharityFactory = await hre.ethers.getContractFactory("BrokenCharity", deployerAcct);
	// 	const brokenCharity_ = await brokenCharityFactory.deploy();
	...
	it("Shouldn't be possible to use an NFT twice for stake/unstake", async function () {
		...
		let newStakingWalletCosmicSignatureNft_ = await stakingWalletCosmicSignatureNftFactory.deploy(
			newCosmicSignatureNftAddr,
			cosmicSignatureGameProxyAddr
		);
		await newStakingWalletCosmicSignatureNft_.waitForDeployment();
		...
		// await hre.ethers.provider.send("evm_mine");
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).unstake(1)).not.to.be.reverted;
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).stake(0)).revertedWithCustomError(newStakingWalletCosmicSignatureNft_, "NftHasAlreadyBeenStaked");

		// await newStakingWalletCosmicSignatureNft_.connect(signer0).doInsertToken(1n, 1n);
		// await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).doInsertToken(1n, 1n)).revertedWithCustomError(newStakingWalletCosmicSignatureNft_, "TokenAlreadyInserted");
		// await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).doRemoveToken(0n)).revertedWithCustomError(newStakingWalletCosmicSignatureNft_, "TokenAlreadyDeleted");
	});
	...
	it("Deposits with value=0 do not create irregularities in StakingWalletCosmicSignatureNft", async function () {
		...
		await hre.ethers.provider.send("evm_increaseTime", [6000]);
		// await hre.ethers.provider.send("evm_mine");
		await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).deposit(0)).not.to.be.reverted; // msg.value = 0
		await newStakingWalletCosmicSignatureNft_.connect(signer0).unstake(1);
		await hre.ethers.provider.send("evm_increaseTime", [100]);
		// await hre.ethers.provider.send("evm_mine");
		// await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).claimManyRewards([1], [0])).not.to.be.reverted;

		await newStakingWalletCosmicSignatureNft_.connect(signer0).stake(1);
		await newStakingWalletCosmicSignatureNft_.connect(signer0).stake(2);
		await hre.ethers.provider.send("evm_increaseTime", [6000]);
		// await hre.ethers.provider.send("evm_mine");
		await newStakingWalletCosmicSignatureNft_.connect(signer0).deposit(1); // msg.value = 0
		// await expect(newStakingWalletCosmicSignatureNft_.connect(signer0).unstakeClaim(3, 1)).not.to.be.reverted;
		await newStakingWalletCosmicSignatureNft_.connect(signer0).unstakeMany([5, 4]);
	});
	...
	// 	await cosmicSignatureGameProxy.connect(ownerAcct).setRoundActivationTime(latestBlock_.timestamp + 1);
	//
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.stake(0);
	//
	...
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.unstake(1);
	// 	await newStakingWalletCosmicSignatureNft_.claimManyRewards([1], [0]);
	//
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.stake(1);
	//
	// 	await hre.ethers.provider.send("evm_increaseTime", [600]);
	// 	// await hre.ethers.provider.send("evm_mine");
	//
	...
	//
	// 	await newStakingWalletCosmicSignatureNft_.>>>connect>>>.unstake(3);
