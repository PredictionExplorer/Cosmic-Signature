...
describe("CosmicSignatureGame-2", function () {
	...
	it("The transferOwnership method", async function () {
		...
		for ( let counter_ = 0; counter_ <= 1; ++ counter_ ) {
			...
			await waitForTransactionReceipt(contracts_.cosmicSignatureGameProxy.connect(contracts_.ownerSigner).setRoundActivationTime((counter_ <= 0) ? 123_456_789_012n : 123n));
			...
			await expect(contracts_.cosmicSignatureGameProxy.connect(contracts_.signers[2]).transferOwnership(contracts_.ownerSigner.address)).revertedWithCustomError(contracts_.cosmicSignatureGameProxy, "OwnableUnauthorizedAccount");
			await waitForTransactionReceipt(contracts_.cosmicSignatureGameProxy.connect(contracts_.ownerSigner).transferOwnership(contracts_.signers[2].address));
			expect(await contracts_.cosmicSignatureGameProxy.owner()).equal(contracts_.signers[2].address);
			await waitForTransactionReceipt(contracts_.cosmicSignatureGameProxy.connect(contracts_.signers[2]).transferOwnership(contracts_.ownerSigner.address));
			expect(await contracts_.cosmicSignatureGameProxy.owner()).equal(contracts_.ownerSigner.address);
			await expect(contracts_.cosmicSignatureGameProxy.connect(contracts_.signers[2]).transferOwnership(contracts_.ownerSigner.address)).revertedWithCustomError(contracts_.cosmicSignatureGameProxy, "OwnableUnauthorizedAccount");
		}
	});
