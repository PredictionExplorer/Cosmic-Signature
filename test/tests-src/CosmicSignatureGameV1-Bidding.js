...
const { testGameRounds, finishGameRound } = require("../src/GameRoundTestHelpers.js");
...
describe("CosmicSignatureGameV1-Bidding", function () {
	it("Smoke-test", async function () {
		...
		expect(ethDutchAuctionDuration_).equal(2n * SECONDS_PER_DAY + 2n);
		...
		expect(cstDutchAuctionDuration_).equal(1n * SECONDS_PER_DAY / 2n);
	});

	// This is a well though out test, but it tests only V1.
	// There are, however, other tests that test other versions.
	it("The halveEthDutchAuctionEndingBidPrice method", async function () {
		...
		for ( let iteration1Counter_ = 1; ; ++ iteration1Counter_ ) {
...

			{
				...
				for ( let iteration2Counter_ = 1; ; ++ iteration2Counter_ ) {
					...
					if (ethDutchAuctionRemainingDuration2_ > 0n) {
						...
						// console.info("%s", `202508161 ${latestBlock_.timestamp}`);
					...
					const nextBlockTimeExtraIncrease_ = generateRandomUInt256() % (((24n + 2n) * SECONDS_PER_HOUR) << BigInt(iteration1Counter_)) - ((2n * SECONDS_PER_HOUR) << BigInt(iteration1Counter_));
					if (nextBlockTimeExtraIncrease_ > 0n) {
						if (nextBlockTimeExtraIncrease_ > 1n) {
							// console.info("%s", "202508204");
							await hre.ethers.provider.send("evm_increaseTime", [Number(nextBlockTimeExtraIncrease_),]);
						} else {
							// console.info("%s", "202508205");
						}
						await hre.ethers.provider.send("evm_mine");
					} else {
						// console.info("%s", "202508206");
					}

					// #endregion
					// #region

					const nextEthBidPrice1_ = await contracts_.cosmicSignatureGameProxy.getNextEthBidPriceAdvanced(1n);
					...
					const nextEthBidPrice2_ = await contracts_.cosmicSignatureGameProxy.getNextEthBidPrice();
					expect(nextEthBidPrice2_).greaterThan(0n);
					const nextEthBidPrice3_ = await contracts_.cosmicSignatureGameProxy.getNextEthBidPriceAdvanced(1n);
...

			{
				...
				for (let roundCounter_ = generateRandomUInt32() % 40; ; ) {
					...
					if ( ! configurationRestored_ ) {
						...
						// console.info("%s", "202508174");
					}

					// Preventing exponential growth of ETH bid price.
					if (nextEthBidPrice_ >= 10n ** (18n - 1n)) {
						// console.info("%s", "202508159");
						break;
					}

					if (( -- roundCounter_ ) < 0) {
						// console.info("%s", "202508175");
						break;
					}
					const roundActivationTime_ = await contracts_.cosmicSignatureGameProxy.roundActivationTime();
					const nextBlockTime_ = Number(roundActivationTime_) + generateRandomUInt32() % (Number(SECONDS_PER_HOUR));
					await hre.ethers.provider.send("evm_setNextBlockTimestamp", [Number(nextBlockTime_),]);
					// await hre.ethers.provider.send("evm_mine");
				}
			}
			...
			// 	} catch {
			// 		console.info("%s", "202508176");
			// 		latestBlockTimeStampAsString_ = latestBlock_.timestamp.toString();
			// 	}
			// 	console.info("%s", `202508148 ${latestBlockTimeStampAsString_}`);
			// }
	...
	it("Bidding-related durations", async function () {
		...
		await testGameRounds(contracts_, contracts_.cosmicSignatureGameProxy, async (contracts_, game_) => {
			...
			let randomWalkNftId_ = await contracts_.randomWalkNft.totalSupply() - 1n;
	...
	it("Bidding with ETH + Random Walk NFT", async function () {
		...
		await testGameRounds(contracts_, contracts_.cosmicSignatureGameProxy, async (contracts_, game_) => {
			...
			let nextEthBidPrice_ = await contracts_.cosmicSignatureGameProxy.getNextEthBidPriceAdvanced(1n);
			let nextEthPlusRandomWalkNftBidPrice_ = await contracts_.cosmicSignatureGameProxy.getEthPlusRandomWalkNftBidPrice(nextEthBidPrice_);
			await expect(contracts_.cosmicSignatureGameProxy.connect(contracts_.signers[5]).bidWithEth(randomWalkNftId_, "hello", {value: nextEthPlusRandomWalkNftBidPrice_,})).revertedWithCustomError(contracts_.cosmicSignatureGameProxy, "CallerIsNotNftOwner");
			nextEthBidPrice_ = await contracts_.cosmicSignatureGameProxy.getNextEthBidPriceAdvanced(1n);
			...
			await waitForTransactionReceipt(contracts_.cosmicSignatureGameProxy.connect(contracts_.signers[1]).bidWithEth(randomWalkNftId_, "hello", {value: nextEthPlusRandomWalkNftBidPrice_,}));
	...
	it("Bidding with CST", async function () {
		...
		let durationUntilMainPrize_ = await contracts_.cosmicSignatureGameProxy.getDurationUntilMainPrize();
		await hre.ethers.provider.send("evm_increaseTime", [Number(durationUntilMainPrize_) - await makeNextBlockTimeDeterministic(600),]);
		// await hre.ethers.provider.send("evm_mine");
		...
		let nextCstBidPrice_ = await contracts_.cosmicSignatureGameProxy.getNextCstBidPrice();
		expect(nextCstBidPrice_).equal(nextCstBidExpectedPrice_);
		++ cstDutchAuctionElapsedDuration_;
		-- cstDutchAuctionRemainingDuration_;
		nextCstBidExpectedPrice_ = nextRoundFirstCstDutchAuctionBeginningBidPrice_ * cstDutchAuctionRemainingDuration_ / cstDutchAuctionDuration_;
