...
describe("MarketingWallet", function () {
	...
	it("Changing the treasurer", async function () {
		...
		const newTreasurerSigner_ = ((randomNumber32_ & 2) == 0) ? contracts_.ownerSigner : contracts_.signers[12];
		...
		await expect(contracts_.marketingWallet.connect(contracts_.ownerSigner).setTreasurerAddress(hre.ethers.ZeroAddress))
			.revertedWithCustomError(contracts_.marketingWallet, "ZeroAddress");
		await expect(contracts_.marketingWallet.connect(contracts_.ownerSigner).setTreasurerAddress(newTreasurerSigner_.address))
	...
	it("Paying marketing rewards", async function () {
		...
		{
			const availableMarketingRewardAmount_ = await contracts_.cosmicSignatureToken.balanceOf(contracts_.marketingWalletAddress);
			let randomNumber32_ = generateRandomUInt32();
			const unauthorizedTreasurerSigner_ = ((randomNumber32_ & 1) == 0) ? contracts_.ownerSigner : contracts_.signers[11];
			...
			await expect(contracts_.marketingWallet.connect(contracts_.treasurerSigner).payReward(hre.ethers.ZeroAddress, 0n))
				.revertedWithCustomError(contracts_.cosmicSignatureToken, "ERC20InvalidReceiver");
			await expect(contracts_.marketingWallet.connect(contracts_.treasurerSigner).payReward(contracts_.signers[1].address, 0n))
				.emit(contracts_.marketingWallet, "RewardPaid")
				.withArgs(contracts_.signers[1].address, 0n);
			await expect(contracts_.marketingWallet.connect(contracts_.treasurerSigner).payReward(contracts_.signers[1].address, 21n))
				.emit(contracts_.marketingWallet, "RewardPaid")
			...
			await expect(contracts_.marketingWallet.connect(unauthorizedTreasurerSigner_)["payManyRewards(address[],uint256)"](marketerAddresses_, 31))
				.revertedWithCustomError(contracts_.marketingWallet, "UnauthorizedCaller")
				.withArgs("Only the tresurer is permitted to call this method.", unauthorizedTreasurerSigner_.address);
			await expect(contracts_.marketingWallet.connect(contracts_.treasurerSigner)["payManyRewards(address[],uint256)"](marketerAddresses_, 31))
				.emit(contracts_.marketingWallet, "RewardPaid")
			...
			await expect(contracts_.marketingWallet.connect(unauthorizedTreasurerSigner_)["payManyRewards((address,uint256)[])"](transferSpecs_))
				.revertedWithCustomError(contracts_.marketingWallet, "UnauthorizedCaller")
				.withArgs("Only the tresurer is permitted to call this method.", unauthorizedTreasurerSigner_.address);
			await expect(contracts_.marketingWallet.connect(contracts_.treasurerSigner)["payManyRewards((address,uint256)[])"](transferSpecs_))
				.emit(contracts_.marketingWallet, "RewardPaid")
			...
			expect(await contracts_.cosmicSignatureToken.balanceOf(contracts_.marketingWalletAddress)).equal(marketingWalletExpectedBalanceAmount_);
		}
	});
});
