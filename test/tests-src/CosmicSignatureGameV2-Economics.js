...
describe("CosmicSignatureGameV2-Economics", function () {
	it("documents zero-price CST bids minting only the computed reward", async function () {
		...
		const reward_ = await game_.getBidCstRewardAmount();
		...
		expect(parsed_.args.bidCstRewardAmount).gte(reward_);
		expect(await contracts_.cosmicSignatureToken.balanceOf(bidder_.address)).equal(balanceBefore_ + parsed_.args.bidCstRewardAmount);
	...
	it("ETH bids can abruptly reduce the current CST bid price to zero near the duration boundary", async function () {
		...
		const duration_ = await game_.cstDutchAuctionDuration();
		await mineAt((await getLatestBlockTimestamp()) + duration_ - 2n);
		...
		expect(priceAfter_).lte(priceBefore_);
