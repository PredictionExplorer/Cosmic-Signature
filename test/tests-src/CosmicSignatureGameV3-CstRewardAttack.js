...
async function deployGameAndHostileBidder() {
	...
	await cstRewardBlockingBidder_.waitForDeployment();
	return { contracts_, game_, cstRewardBlockingBidder_ };
}
...
async function hostileBidWithEthAt(game_, cstRewardBlockingBidder_, callerSigner_, timeStamp_) {
	const ethBidPrice_ = await game_.getNextEthBidPriceAdvanced(timeStamp_ - await getLatestBlockTimestamp());
	await hre.ethers.provider.send("evm_setNextBlockTimestamp", [Number(timeStamp_),]);
	const receipt_ = await waitForTransactionReceipt(
		cstRewardBlockingBidder_.connect(callerSigner_).doBidWithEth(-1n, "hostile bid", 0n, {value: ethBidPrice_,})
	);
	expect(await blockTimestampOfReceipt(receipt_)).equal(timeStamp_);
	return receipt_;
}
...
describe("CosmicSignatureGameV3-CstRewardAttack", function () {
	it("a bidder contract that rejects all incoming calls cannot block subsequent ETH or CST bids", async function () {
		...
		const cstRewardBlockingBidderAddress_ = await cstRewardBlockingBidder_.getAddress();
		...
		for (const hostilityModeCode_ of [1n, 2n, 3n,]) {
			await waitForTransactionReceipt(cstRewardBlockingBidder_.setHostilityModeCode(hostilityModeCode_));
			...
			await hostileBidWithEthAt(game_, cstRewardBlockingBidder_, deployerOfHostileContract_, lastBidTimeStamp_);
			expect(await game_.lastBidderAddress()).equal(cstRewardBlockingBidderAddress_);

			// Sanity check: the hostile contract really is hostile - a plain ETH transfer to it fails.
			await expect(
				eoaBidder1_.sendTransaction({to: cstRewardBlockingBidderAddress_, value: 1n, gasLimit: 1_000_000n,})
			).reverted;
			...
			// with a recipient callback, this would revert; it must succeed, minting 90% to the hostile contract.
			{
				...
				expect(
					await token_.balanceOf(cstRewardBlockingBidderAddress_) - hostileCstBalanceBefore_,
		...
		// minted; no ETH or callback touches it).
		{
			...
			await hre.ethers.provider.send("evm_setNextBlockTimestamp", [Number(bidTimeStamp_),]);
			const receipt_ = await waitForTransactionReceipt(
				cstRewardBlockingBidder_.connect(deployerOfHostileContract_).doBidWithCst((1n << 255n), "hostile CST bid", 0n)
			);
			expect(await blockTimestampOfReceipt(receipt_)).equal(bidTimeStamp_);
	...
	it("a reentrancy attempt via the ETH bid overpayment refund reverts the attacker's own bid only", async function () {
		...
		await waitForTransactionReceipt(cstRewardBlockingBidder_.setHostilityModeCode(4n));
		const ethBidPrice_ = await game_.getNextEthBidPriceAdvanced(2n);
		await expect(
			cstRewardBlockingBidder_.connect(deployerOfHostileContract_).doBidWithEth(-1n, "reentry attempt", 0n, {value: ethBidPrice_ + 10n ** 18n,})
	...
	it("a hostile last bidder cannot block the round from completing", async function () {
		...
		await bidWithEthAt(game_, eoaBidder1_, (await getLatestBlockTimestamp()) + 10n);
		await waitForTransactionReceipt(cstRewardBlockingBidder_.setHostilityModeCode(1n));
		await hostileBidWithEthAt(game_, cstRewardBlockingBidder_, deployerOfHostileContract_, (await getLatestBlockTimestamp()) + 30n);
		expect(await game_.lastBidderAddress()).equal(cstRewardBlockingBidderAddress_);
		...
		const hostileCstBalanceChange_ = await token_.balanceOf(cstRewardBlockingBidderAddress_) - hostileCstBalanceBefore_;
