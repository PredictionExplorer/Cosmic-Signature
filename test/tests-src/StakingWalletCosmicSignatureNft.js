...
describe("StakingWalletCosmicSignatureNft", function () {
	...
	it("Minting, staking, and unstaking of at least 10 Cosmic Signature NFTs", async function () {
		...
		for ( let roundNum_ = 0n; ; ++ roundNum_ ) {
			...
			const cosmicSignatureNftNftMintedLogs_ = transactionReceipt_.logs.filter((log_) => (log_.topics[0] == cosmicSignatureNftNftMintedTopicHash_));
		...
		for (let nftIndex_ = nftIds_.length; ( -- nftIndex_ ) >= 0; ) {
			...
			const transactionReceipt_ = await waitForTransactionReceipt(transactionResponsePromise_);
			const stakingWalletCosmicSignatureNftNftStakedLog_ = transactionReceipt_.logs.find((log_) => (log_.topics[0] == stakingWalletCosmicSignatureNftNftStakedTopicHash_));
	...
	it("The stakeMany and unstakeMany methods", async function () {
...

		{
			...
			for (const cosmicSignatureNftNftMintedLog_ of cosmicSignatureNftNftMintedLogs_) {
				const cosmicSignatureNftNftMintedParsedLog_ = contracts_.cosmicSignatureNft.interface.parseLog(cosmicSignatureNftNftMintedLog_);
				// console.info("%s", `202507211 ${cosmicSignatureNftNftMintedParsedLog_.args.nftId}`);
				nftIds_.push(cosmicSignatureNftNftMintedParsedLog_.args.nftId);
			}
		}
...

		{
			...
			const stakingWalletCosmicSignatureNftNftStakedLogs_ = transactionReceipt_.logs.filter((log_) => (log_.topics[0] == stakingWalletCosmicSignatureNftNftStakedTopicHash_));
			...
			for (let stakingWalletCosmicSignatureNftNftStakedLogIndex_ = stakingWalletCosmicSignatureNftNftStakedLogs_.length; ( -- stakingWalletCosmicSignatureNftNftStakedLogIndex_ ) >= 0; ) {
				...
				// console.info("%s", `202507212 ${stakingWalletCosmicSignatureNftNftStakedParsedLog_.args.stakeActionId} ${stakeActionRecord_}`);
...

		{
			...
			const stakingWalletCosmicSignatureNftNftUnstakedLogs_ = transactionReceipt_.logs.filter((log_) => (log_.topics[0] == stakingWalletCosmicSignatureNftNftUnstakedTopicHash_));
	...
	it("Cosmic Signature NFT staking ETH reward amounts and the tryPerformMaintenance method", async function () {
		...
		for ( let iterationCounter_ = 1; iterationCounter_ <= numIterations_; ++ iterationCounter_ ) {
			...
			for (;;) {
				...
				// Comment-202606179 applies.
				await setRoundActivationTimeIfNeeded(contracts_.cosmicSignatureGameProxy.connect(contracts_.ownerSigner), (nextEthBidPrice_ < 10n ** (18n - 2n)) ? 2n : (-999_999n));
				...
				const stakingWalletCosmicSignatureNftEthDepositReceivedLog_ = transactionReceipt_.logs.find((log_) => (log_.topics[0] == stakingWalletCosmicSignatureNftEthDepositReceivedTopicHash_));
				if (roundNum_ % BigInt(numRoundsPerIteration_) == 0n) {
					// console.info("%s", "202507216");
			...
			// console.info("%s", `202507207 ${stakeActions_.length} ${remainderEthAmount_} ${hre.ethers.formatEther(ethDutchAuctionBeginningBidPrice_)}`);
	...
	it("Bidding and Cosmic Signature NFT staking", async function () {
		...
		for ( let nftId_ = 0n; nftId_ < cosmicSignatureNftTotalSupply_; ++ nftId_ ) {
			...
			transactionReceipt_ = await waitForTransactionReceipt(transactionResponsePromise_);
			log_ = transactionReceipt_.logs.find((log_) => (log_.topics[0] == stakingWalletCosmicSignatureNftNftStakedTopicHash_));
		...
		let cosmicSignatureGameProxyRaffleWinnerPrizePaidLogs_ = transactionReceipt_.logs.filter((log_) => (log_.topics[0] == cosmicSignatureGameProxyRaffleWinnerPrizePaidTopicHash_));
	...
	it("Double-unstaking Cosmic Signature NFTs", async function () {
...

		{
			...
			const transactionReceipt_ = await waitForTransactionReceipt(transactionResponsePromise_);
			const cosmicSignatureNftNftMintedLogs_ = transactionReceipt_.logs.filter((log_) => (log_.topics[0] == cosmicSignatureNftNftMintedTopicHash_));
			// console.info("%s", `202507281 ${cosmicSignatureNftNftMintedLogs_.length}`);
			for (const cosmicSignatureNftNftMintedLog_ of cosmicSignatureNftNftMintedLogs_) {
				const cosmicSignatureNftNftMintedParsedLog_ = contracts_.cosmicSignatureNft.interface.parseLog(cosmicSignatureNftNftMintedLog_);
				// console.info("%s", `202507282 ${cosmicSignatureNftNftMintedParsedLog_.args.nftId}`);
				nftIds_.push(cosmicSignatureNftNftMintedParsedLog_.args.nftId);
			}
		}
...

		{
			...
			for (const stakingWalletCosmicSignatureNftNftStakedLog_ of stakingWalletCosmicSignatureNftNftStakedLogs_) {
				const stakingWalletCosmicSignatureNftNftStakedParsedLog_ = contracts_.stakingWalletCosmicSignatureNft.interface.parseLog(stakingWalletCosmicSignatureNftNftStakedLog_);
				// console.info("%s", `202507283 ${stakingWalletCosmicSignatureNftNftStakedParsedLog_.args.nftId} ${stakingWalletCosmicSignatureNftNftStakedParsedLog_.args.stakeActionId}`);
		...
		for (const stakeActionId_ of duplicatedStakeActionIds_) {
			...
			if ( ! unstakedStakeActionIds_[stakeActionId_] ) {
				// console.info("%s", `202507284 ${stakeActionId_}`);
				...
				// console.info("%s", `202507285 ${stakeActionId_}`);
