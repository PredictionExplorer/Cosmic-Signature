...
describe("CosmicSignatureGame-All-Versions", function () {
	...
	it("The transferOwnership method", async function () {
		await testAcrossGameVersions(async (contracts_, game_) => {
			...
			const gameImplementationAddress_ = await hre.upgrades.erc1967.getImplementationAddress(await game_.getAddress());
			const gameImplementation_ = game_.attach(gameImplementationAddress_);
			expect(await gameImplementation_.owner()).equal(hre.ethers.ZeroAddress);
			await expect(gameImplementation_.connect(contracts_.ownerSigner).transferOwnership(contracts_.deployerSigner.address)).revertedWithCustomError(gameImplementation_, "OwnableUnauthorizedAccount");
			await expect(gameImplementation_.connect(contracts_.deployerSigner).transferOwnership(contracts_.ownerSigner.address)).revertedWithCustomError(gameImplementation_, "OwnableUnauthorizedAccount");
			...
			for ( let counter_ = 0; counter_ <= 1; ++ counter_ ) {
				...
				await waitForTransactionReceipt(game_.connect(contracts_.ownerSigner).setRoundActivationTime((counter_ <= 0) ? 123n : 123_456_789_012n));
