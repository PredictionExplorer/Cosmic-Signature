...
describe("CosmicSignatureNft", function () {
	...
	it("NFT minting", async function () {
...

		{
			...
			expect((await newCosmicSignatureNft_.getNftInfo(nftId_)).seed).equal(nftSeed_);
	...
	it("The getNftName and setNftName methods ", async function () {
...

		{
			...
			do {
				...
				expect((await contracts_.cosmicSignatureNft.getNftInfo(nftId_)).name).equal(newNftName_);
