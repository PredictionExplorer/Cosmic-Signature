...
describe("Cosmic Set1", function () {
	...
	it("donateWithInfo() works as expected", async function () {
		...
		await cosmicGame.connect(addr1).donateWithInfo(dataStr,{ value: donationAmount });
		let numDonationInfoRecs = await cosmicGame.donateWithInfoNumRecords();
		expect(numDonationInfoRecs).to.equal(1);
		let donationInfoRec = await cosmicGame.getDonationInfoRecord(0);
		...
		await cosmicGame.connect(addr1).donateWithInfo(dataStr,{ value: donationAmount });
		numDonationInfoRecs = await cosmicGame.donateWithInfoNumRecords();
