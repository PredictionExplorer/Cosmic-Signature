...
- Slots 309-312: `roundLateBidDurationDivisor`, `roundLateBidPricePremiumAmountBaseMultiplier`, `roundLateBidPricePremiumAmountExponent`, `mainPrizeNumCosmicSignatureNfts`. Slot 313: the `bidRaffleCumulativeWeights` mapping.
- The gap shrinks to `(1 << 30) - 1 - 6`; the end-of-gap position is exactly preserved (V2 gap: slots 308..306+2^30; V3 gap: slots 314..306+2^30).
- **No slots are renamed or repurposed.** `cstDutchAuctionDuration`, `cstDutchAuctionDurationChangeDivisor` keep their slots and their live values, and V3 keeps using them exactly like V2 did (Comment-202606101, Comment-202608301). `bidCstRewardAmountMultiplier`, `cstDutchAuctionBeginningBidPriceMinLimit`, and the five ETH prize percentages keep their slots but are re-initialized by the V3 `reinitialize` (see section 3).
...
- Getters for the 6 new storage variables listed in section 1 (incl. `championDurations(uint256)`).
- `getRoundLateBidDuration() view returns (uint256)`.
- Setters: `setRoundLateBidDurationDivisor`, `setRoundLateBidPricePremiumAmountBaseMultiplier`, `setRoundLateBidPricePremiumAmountExponent`, `setMainPrizeNumCosmicSignatureNfts`.
- Events: `RoundLateBidDurationDivisorChanged`, `RoundLateBidPricePremiumAmountBaseMultiplierChanged`, `RoundLateBidPricePremiumAmountExponentChanged`, `MainPrizeNumCosmicSignatureNftsChanged`.
- Errors: `BidPlacedWithinCurrentSecond()`, `ZeroValue()`.
...
- `BidPlaced` keeps its signature (topic0 unchanged); the 8th parameter, `cstDutchAuctionDuration`, is identical to V2 in both name and meaning. Only the 7th parameter (`bidCstRewardAmount`) changes meaning (it is now minted to the previous bidder).
...
1. **CST Dutch auction: unchanged V2 duration mechanics.** The stored `cstDutchAuctionDuration` keeps driving the price (`beginningPrice * remaining / duration`), shrinking ~0.4% on every ETH bid and growing ~0.4% on every CST bid (`cstDutchAuctionDurationChangeDivisor = 250`), exactly like V2 (Comment-202606101); nothing else moves it. Both V2 setters remain functional. The only CST-auction change is the beginning-price minimum dropping from 200 CST to 1 CST at `reinitialize` time.
...
8. **Configuration setter validation (Comment-202608171).** `setMainPrizeNumCosmicSignatureNfts`, `setRoundLateBidDurationDivisor`, and `setRoundLateBidPricePremiumAmountExponent` reject a zero with the new `ZeroValue` error. Rationale: a zero `mainPrizeNumCosmicSignatureNfts` would have permanently bricked the game once a bid exists (claim reverts with `Panic(0x11)` and neither repair nor upgrade is possible while the round is active); a zero `roundLateBidDurationDivisor` would have frozen bidding with `Panic(0x12)` for the rest of the round. Zero remains valid for `setRoundLateBidPricePremiumAmountBaseMultiplier` (disables the premium) and `setBidCstRewardAmountMultiplier` (disables rewards), and the `SystemManagementV2`-inherited setters keep accepting any value, like in V2.
...
- **Fork rehearsal (Comment-202608128): PASSING.** `tasks/src/fork-rehearse-cosmic-signature-game-v3-upgrade.js`
  ...
  `upgradeToAndCall` + `reinitialize`, and verifies: implementation and initialized-version change; all
  remaining carried-over storage getters unchanged (including the live `cstDutchAuctionDuration` values);
  the 7 intentional `reinitialize` overwrites and 4 new parameters at the
  expected defaults; the five ETH prize percentages total 50% and the raffle count remains 3;
  `InvalidInitialization` on a reinitialize replay; the V2 CST Dutch auction duration setters remain functional;
