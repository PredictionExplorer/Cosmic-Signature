todo-ai-0 todo-0 This doc is currently stale. Do not update it just yet. Update it later.
...
- **Split.** When a bid is placed, `90%` of the reward (`total * 90 / 100`, `DEFAULT_LAST_BIDDER_BID_CST_REWARD_AMOUNT_PERCENTAGE`) is minted to the bidder being outbid (the current `lastBidderAddress`), and the remainder (~10%, plus any rounding dust) is minted to the new bidder. The percentage is a compile-time constant, not a parameter.
