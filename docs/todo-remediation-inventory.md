...
- `docs/cosmic-signature-contracts-audit-considerations.md`: closed the stale `todo-0` about `FuzzTestV2.js`; V1 and V2 are covered by the unified fuzz campaign. Yuriy's comment: I edited/removed parts of that.
- `contracts/production/Bidding.sol`, `contracts/production/BiddingV2.sol`, `contracts/production/CosmicSignatureToken.sol`: closed Comment-202606074 by treating a zero signed `mintAndBurnMany` delta as a zero burn. Yuriy's comment: I have reverted that refactoring and edited and moved Comment-202606074 to the CST contract; the CST contract has already been deployed, so it's impossible to fix it.
- `contracts/production/SystemManagement.sol`, `contracts/production/SystemManagementV2.sol`: closed the Comment-202504212 CST-floor drift by raising `nextRoundFirstCstDutchAuctionBeginningBidPrice` when the owner raises the minimum. Yuriy's comment: I have reverted that refactroring and wrote Comment-202607016. @Taras, does it make sense to you?
- `contracts/production/Bidding.sol`, `contracts/production/BiddingV2.sol`: handled Comment-202606216 by swallowing ETH bid refunds when `tx.gasprice == 0`, matching Arbitrum gas-estimation behavior. Yuriy's comment: I have reverted that refactoring and wrote Comment-202607014.
- `contracts/production/CosmicSignatureGameStorage.sol`: replaced the broad storage-collision TODO with a concrete invariant for future write paths. Yuriy's comment: I removed that comment. It's unnecessary. It really appears that even if there were writes to mappings at arbitrary user supplied positions, there would still be no way someone could inflict damage, right?
...
- `test/tests-src/PrizesWallet-1.js`: console severity/formatting review. Yuriy's comment: I deleted that logging together with the todo near it.
